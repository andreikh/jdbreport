 ...
 * JDBReport Generator
 *
 * Copyright (C) 2004-2018 Andrey Kholmanskih
 *
...
import jdbreport.view.clipboard.ReportTransferable;
...
public class JReportGrid extends JTable implements TableRowModelListener,
    ...
    private void pushRowMovedUndo(int fromIndex, int toIndex) {
     ...
     *
     * @param rowModel the TableRowModel
     */
     ...
     *
     * @param index the CellStyle's id
     ...
     *
     * @param align the new horizontal alignment for the cells
     */
     ...
     *
     * @param align the new vertical alignment for the cells
     */
     ...
     *
     * @param positions positions - boolean values, where true sets border, otherwise
     ...
     *
     * @param fontStyle the font's style The style argument is an integer bitmask that
     ...
     *                  may be PLAIN, or a bitwise union of BOLD, ITALIC, UNDERLINE,
     *                  STRIKETHROUGH
     * @param enable    if true, the style is determined, otherwise the style is
     *                  removed.
     */
     ...
     *
     * @param color the new background color for the cells
     */
    ...
    /**
     * Set or unset round to significant property for selected cells.
     */
    public void roundToSignificant() {
        GridRect rect = getSelectionRect();
        if (rect == null)
            return;
        if (canUndo())
            pushUndo(new StyleUndoItem(this, UndoItem.ROUND_TO_SIGNIFICANT));
        getReportModel().setRoundToSignificant(rect,
                !getCellStyle(getSelectedCell().getStyleId()).isRoundToSignificant());
        repaint();
    }
    ...
    public static class TextReportRenderer extends AbstractReportRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table,
            ...
            String str = formatter.format(value, style.getDecimal(), style.isRoundToSignificant());
    ...
    private static class ValueFormatter {
        ...
        public String format(Object value, int decimal, boolean roundToSignificant) {
            ...
            if (decimal >= 0) {
                try {
                    String s = format(value instanceof Double ? (Double) value : new Double(value.toString().replace(',', '.')),
                            decimal);
                    if (roundToSignificant) {
                        s = Utils.truncateToSignificant(s);
                    }
                    return s;
            ...
            return format(value, roundToSignificant);
        }

        private String formatFloating(Number value, boolean roundToSignificant) {
            String v = numberFormat.format(value);
            if (Utils.getDecimalSeparator() != '.') {
                v = v.replace('.', Utils.getDecimalSeparator());
            }
            if (roundToSignificant) {
                v = Utils.truncateToSignificant(v);
            }
            return v;
        }

        private String format(Object value, boolean roundToSignificant) {
            if (value instanceof Double) {
                return formatFloating((Double) value, roundToSignificant);
            } else if (value instanceof Float) {
                return formatFloating((Float) value, roundToSignificant);
            } else if (value instanceof BigDecimal) {
                return formatFloating((BigDecimal) value, roundToSignificant);
            }

            if (roundToSignificant && value instanceof String) {
                try {
                    String v = (String) value;
                    v = v.replace(',', '.');
                    Double.parseDouble(v);
                    return Utils.truncateToSignificant((String) value);
                } catch (Exception ignore) {}
            }
