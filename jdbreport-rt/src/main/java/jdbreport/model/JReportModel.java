...
public class JReportModel extends AbstractTableModel implements ReportModel {
	...
	protected TableRowModel createRowModel() {
		return new ReportRowModel(this);
	}
	...
	public Object addStyle(CellStyle style) {
		...
		Integer id = new Integer(getStyleList().size());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see jdbreport.model.ReportModel#isBold(int, int)
	 */
	public boolean isBold(int row, int column) {
		CellStyle cellStyle = getStyles(getReportCell(row, column).getStyleId());
		return (cellStyle != null && cellStyle.isBold());
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see jdbreport.model.ReportModel#isItalic(int, int)
	 */
	public boolean isItalic(int row, int column) {
		CellStyle cellStyle = getStyles(getReportCell(row, column).getStyleId());
		return (cellStyle != null && cellStyle.isItalic());
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see jdbreport.model.ReportModel#isUnderline(int, int)
	 */
	public boolean isUnderline(int row, int column) {
		CellStyle cellStyle = getStyles(getReportCell(row, column).getStyleId());
		return (cellStyle != null && cellStyle.isUnderline());
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see jdbreport.model.ReportModel#isStrikethrough(int, int)
	 */
	public boolean isStrikethrough(int row, int column) {
		CellStyle cellStyle = getStyles(getReportCell(row, column).getStyleId());
		return (cellStyle != null && cellStyle.isStrikethrough());
	}
	...
	public boolean isColumnBreak(int column) {
		...
		if (reportColumn instanceof ReportColumn) {
			return ((ReportColumn) reportColumn).isPageBreak();
		}
		return false;
	}
