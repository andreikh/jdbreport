...
int reportPageHeight = (int) pageFormat
		.getImageableHeight();
...
int rowMargin = model.getRowModel().getRowMargin();

int leftCol = 0;
do {
	int rightCol = model.findRightColumn(leftCol);
	int columnCount = rightCol - leftCol + 1;
	...
	for (int row = 0; row < rowModel.getRowCount(); row++) {
		...
		rowHeights += reportRow.getHeight() + rowMargin;

		if (model.isLastRowInPage(row)
				|| (row < rowModel.getRowCount() - 1 && rowHeights
						+ rowModel.getRow(row + 1)
								.getHeight() > reportPageHeight)) {
			document.add(table);
			table = createPdfTable(columnCount, widths,
	...
	if (leftCol < model.getColumnCount()) {
		document.newPage();
	}
