...
public class JReportModel extends AbstractTableModel implements ReportModel {
    ...
    public Dimension getCellSize(Cell cell, int row, int column, boolean includeSpacing) {
     ...
     *
     * @param styleId the identifier of style
     ...
     * Replaces styles
     *
     * @param oldStyleId old style's id
     * @param newStyleId new style's id
     */
    ...
    public boolean isHideFirstHeader() {
        return getRowModel().isHideFirstHeader();
    }

    public void setHideFirstHeader(boolean b) {
        getRowModel().setHideFirstHeader(b);
    }
    ...
    public void updatePages(int startRow) {
        ...
        for (int c = 0; c < cm.getColumnCount(); c++) {
            ...
            if (column.isPageBreak()) {
                ...
                w = 0;
            } else if (!getReportPage().isShrinkWidth()) {
                if (w > pageWidth && c > 0) {
