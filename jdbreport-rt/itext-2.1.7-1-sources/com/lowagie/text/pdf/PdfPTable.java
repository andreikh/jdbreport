 ...
 *
 * modified by Andrey Kholmanskih 14.04.2011
 */
...
public class PdfPTable implements LargeElement{
    ...
    /**
     * Changed by Andrey Kholmanskih 14.04.2011 
     * 
    ...
    boolean rowSpanAbove(int currRow, int currCol) {
    	...
    	while (row > 0) {
    		...
    		int col = currCol;
    		while (col >= 0) {
    			PdfPCell cell = (PdfPCell)aboveRow.getCells()[col];
    			if (cell != null && cell.getColspan() + col > currCol &&
    					row + cell.getRowspan() > currRow) {
    				return true;
    			}
    			col--;
    		}
    ...
    public float getRowHeight(int idx, boolean firsttime) {
        ...
        for (int i = 0; i < relativeWidths.length; i++) {
        	...
        	//Changed 27.11.2010 by Andrey Kholmanskih
			if (cell != null) {
				float tmp = 0;
